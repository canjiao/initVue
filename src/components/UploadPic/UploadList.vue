  ...
  <div>
    ...
    >
      <template v-if="item.status === 'finished'">
        <img :src="item.url">
          <viewer :options="options" v-if="showViewer"
              @inited="inited"
    ...
    <Modal
      title="查看"
      v-model="proudctImage.visible"
      class="view-modal"
    >
      <img
        :src="proudctImage.curPath"
        v-if="proudctImage.visible"
        style="width: 100%"
      />
    </Modal>
  </div>
...
export default {
  ...
  methods: {
    ...
    show (e) {
      let index = $(e.target).parents('.imageUpload').find('.demo-upload-list').index($(e.target).parents('.demo-upload-list'))
      this.$viewer.show()
